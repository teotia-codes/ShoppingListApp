    ...
    Column(
        ...
        LazyColumn(
            ...
            items(sItems) {
              item ->
                if(item.isEditing){
                    ShoppingDialog(item = item, editingComplete ={
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ShoppingDialog(item: ShoppingItem,editingComplete: (String, Int) ->  Unit,id: Int) {
        ...
        AlertDialog(onDismissRequest = { showDialog = false}, confirmButton = {
            Row (modifier = Modifier
                ...
                Button(onClick = {
                    isEditing = false
                    showDialog = false
                    editingComplete(editedName, editedQuantity.toIntOrNull() ?: 1)
                  }) {
                    Text(text = "Add")
